...
if __name__ == '__main__':
	# 设置目标数据库
	# mysql or sqlite or mongodb or excel
	database = "sqlite"
	# database = "mongodb"
	# database = "excel"
	# database = "sqlite"
	...
	if database == "mysql":
		...
		conn2 = sqlite3.connect('ibf_estate.db')
		#db2 = conn.cursor()
			...
			with open(csv, 'r') as f:
				for line in f:
					...
					# 写入sqlite数据库
						...
						sqlStr = "INSERT OR IGNORE INTO ibf_lianjia_xiaoqu (city, date, district, area, xiaoqu, price, sale) VALUES ('%s','%s','%s', '%s', '%s', %s, %s)" %(city_ch, date, district, area, xiaoqu, price, sale)
						try:
							db2 = conn2.cursor()
							db2.execute(sqlStr)
							conn2.commit()
